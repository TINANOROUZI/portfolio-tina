...
export default function OverlayMenu({ open, onClose }) {
  ...
  return (
    ...
    >
      ...
      {/* CONTENT
         - On mobile: stack (menu only), stripes are hidden
         - On ≥sm: show stripes at left and menu at right */}
      <div className="flex h-[calc(100%-80px)] flex-col sm:flex-row">
        {/* LEFT STRIPES (hidden on mobile to prevent overlap) */}
        <div className="hidden sm:flex w-[38%] max-w-[360px] h-full">
          {stripes.map((s, i) => (
        ...
        <div className="flex-1 grid place-items-center px-6 py-6">
          <ul className="w-full max-w-[540px] space-y-6 sm:space-y-5">
            {menu.map((m, i) => (
              <li
                key={m.label}
                className="reveal"
              ...
              >
                {"to" in m ? (
                  <Link
                    to={m.to}
                    onClick={onClose}
                    /* responsive text size: huge on desktop, scaled on phones */
                    className="menu-link font-stencil text-[11vw] leading-[1.04] sm:text-6xl md:text-7xl"
                  >
                    <span
                      className={`pointer mr-2 sm:mr-3 ${
